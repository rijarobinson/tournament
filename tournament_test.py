...
def testStandingsBeforeMatches():
    ...
    if len(standings) < 2:
        ...
        raise ValueError("Only registered players should appear in standings. Number standings in your function: " + str(standings))
    if len(standings[0]) != 4:
        raise ValueError("Each playerStandings row should have four columns. Your function returns: " + str(standings))
    [(id1, name1, wins1, matches1), (id2, name2, wins2, matches2)] = standings
    if matches1 != 0 or matches2 != 0 or wins1 != 0 or wins2 != 0:
        raise ValueError(
            "Newly registered players should have no matches or wins. Function returns: " + str(standings))
...
def testReportMatches():
    ...
    for (i, n, w, m) in standings:
        if m != 1:
            raise ValueError("Each player should have one match recorded. Your function returns: %s" % str(m))
    ...
    for (i, n, w, m) in standings:
        if m != 0:
            raise ValueError("After deleting matches, players should have zero matches recorded. You have: %s" % str(m))
        if w != 0:
